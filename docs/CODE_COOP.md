...
## 2026-06-11 — P2.6 Stage 13 DB integration (rusqlite engine, query builder, schema viewer, codegen)

Branch worktree `agent-adb3b11803f47b733`, targeting `dev`. Implemented full P2.6 DB integration: (1) Added `rusqlite = { version = "0.40", features = ["bundled"] }` to `Cargo.toml`. (2) Created `src/project/db_engine.rs` — `DatabaseEngine` trait with `connect/list_tables/list_columns/preview_rows`, `SqliteEngine` impl using `rusqlite`, identifier validation guard (Invariant 10), 6 unit tests. (3) Created `src/panels/db_panel.rs` — floating "Database" window: path TextEdit + Connect + Refresh, schema tree (tables → columns), query builder (table ComboBox + limit + Preview button, egui::Grid row display). (4) Added `DbBinding { table, column }` to `schema.rs` with `#[serde(default)]` + serde round-trip test. (5) Extended `state_emitter.rs` to emit `db_conn: Option<rusqlite::Connection>` and `load_from_db()` stub when any widget has a DB binding. (6) Added Invariant 10 to `docs/ENGINEERING_INVARIANTS.md`. (7) Wired `db_engine` and `db_panel` fields into `RohKaiApp`, added "Database…" View menu item. 423 tests pass, zero clippy warnings. Next agent: merge this worktree into `dev`; watch for merge conflicts in `schema.rs`/`properties.rs`/`app.rs` if P2.3–P2.5 agents also touched those files.
