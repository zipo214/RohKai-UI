...
## 2026-06-11 — P2.5: formula depth + timer wiring + state machine + shortcuts + .rkwb bundle

Worktree agent on `dev`. Implemented all five P2.5 items: (1) `formula.rs` — `deps()` alias for `collect_variables`, `validate()` alias for `parse_formula`; both wired into the MathLabel properties panel; tests `formula_validate_catches_syntax_error` + `formula_deps_finds_bindings`. (2) Timer wiring — `timer_rx: Option<mpsc::Receiver<String>>` + `timers_need_respawn: bool` on `RohKaiApp`; `spawn_timers()` spawns a `std::thread` per Timer component; drain loop in `update()`; respawned after new/open. (3) `StateMachineProps`/`StateDef`/`TransitionDef` added to `schema.rs`; `DesignComponent` gains `state_machine: StateMachineProps` field; component_tray `show_config` shows states + transitions table UI + initial state selector; codegen emits `String` current_state field with initial value. (4) `user_shortcuts: HashMap<String, String>` added to `UserSettings`; shortcuts window gains two-tab layout (Reference/Customize) with editable key-combo fields and reset buttons; serde round-trip test. (5) `widget_bundle.rs` gains `build_bundle()` — ZIP store-only (method=0, std::io only) with CRC-32 IEEE, per-descriptor `.rkwd` entries, `manifest.json`; test asserts PK magic bytes. 424 tests, zero clippy warnings. Next agent: merge this worktree to dev after verifying no conflicts with P2.3/P2.4/P2.6/P2.7 agents.
