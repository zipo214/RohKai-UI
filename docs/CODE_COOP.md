...
## 2026-06-11 — P2.4: layout UX depth (per-child alignment, drag-reorder, layout-aware hit-test)

Branch `worktree-agent-a82f07243b60ac92b` (will merge to `dev`). Implemented all four P2.4 items: (1) `CrossAlign` enum + `child_flex`/`grid_col_span`/`grid_row_span` fields on `WidgetInstance` with `#[serde(default)]` skip-at-defaults — compact JSON, zero backward-compat breakage. (2) Properties panel shows "Layout child" section with cross-axis align buttons, flex drag-value, and grid span controls whenever the selected widget has a VLayout/HLayout/GridLayout parent. (3) `ReorderDrag` state + `layout_insert_idx` + `draw_insertion_placeholder` in `src/canvas/interaction.rs` — dragging a VLayout/HLayout child shows an accent-color insertion line and commits `move_child_within_parent` on drop. (4) `find_layout_container_at` restricts rubber-band selection to direct children when the band starts inside a layout container. Three new schema tests added; 419 total, zero warnings. ROADMAP_PHASE2.md P2.4 all [x]. Next agent: merge this worktree commit into `dev` and then address remaining Phase 2 work (P2.3 constraint layout, P2.5 widget depth, P2.6 DB, P2.7 SVG lanes). Watch for possible schema conflicts with P2.3 agent if it also touched `WidgetInstance`.
